...
@Component({
  selector: 'defect-table',
  encapsulation: ViewEncapsulation.None,
  styleUrls: ['./defectTable.scss'],
  templateUrl: './defectTable.html'
})
export class DefectTable {
  ...
  constructor(
    protected service: DefectService,
  ...
  loadData() {
    this.service
      .getSectionAndDefectDateBetweenPage(0, '1970-01-01', '2100-12-31', 0, 20)
      .subscribe((data: any) => {
        this.fillTable(data);
      });
  }

  lazy(event: any, dt:any) {
    console.log(event);
  ...
  search(first: number, pageSize: number): void {
    ...
    this.service
      .getSectionAndDefectDateBetweenPage(
  ...
  onRowDblclick(data: any): void {
    window.open('/#/pages/defect/form/' + data.id, '_blank');
  }

  navigateToForm(id: any): void {
    this.router.navigate(['/pages/defect/form/' + id]);
  }
